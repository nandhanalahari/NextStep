...
export default function LoginPage() {
  ...
  useEffect(() => {
    ...
    if (error === "denied") toast.error("Sign in with Google was cancelled.")
    else if (error === "config") toast.error("Google sign-in is not configured. Add GOOGLE_CLIENT_ID and GOOGLE_REDIRECT_URI.")
    else if (error === "no_email") toast.error("Google did not provide an email address.")
    else if (error) toast.error("Something went wrong with Google sign-in. Try again.")
  }, [searchParams])
          ...
          <div className="relative rounded-2xl border border-primary/20 bg-card p-8">
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-5">
              ...
              <Button
                type="submit"
                disabled={loading}
                className="mt-2 bg-primary text-primary-foreground hover:bg-primary/90 h-11 font-medium disabled:opacity-70"
              >
            ...
            <div className="mt-6 flex flex-col gap-3">
              <Button
                variant="outline"
                className="h-11 border-border/50 bg-transparent text-foreground hover:bg-secondary"
                asChild
              >
                <a
                  href={`/api/auth/google-signin${searchParams.get("from") ? `?from=${encodeURIComponent(searchParams.get("from")!)}` : ""}`}
                >
